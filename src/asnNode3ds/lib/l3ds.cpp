...
//ErrorMsg("Rnd::Try goto chunk...\n");

    GotoChunk(edit);
    obj.id = MAT_ENTRY;
    while (FindChunk(obj, edit))
    {
//        ErrorMsg("Rnd::Read Material...\n");
        ReadMaterial(obj);
//        ErrorMsg("Rnd::Skip chunk...\n");
        SkipChunk(obj);
    }

//ErrorMsg("Rnd::Goto chunk edit...\n");
...
//        ErrorMsg("Rnd::ReadASCIIZ...\n");
            ReadASCIIZ(m_objName, 99);
            ml = ReadChunk();
            if (ml.id == OBJ_TRIMESH) {
//        ErrorMsg("Rnd::Read Mesh...\n");
                ReadMesh(ml); }
			else
            if (ml.id == OBJ_LIGHT) {
//        ErrorMsg("Rnd::Read Light...\n");
                ReadLight(ml); }
			else
            if (ml.id == OBJ_CAMERA) {
//        ErrorMsg("Rnd::Read Camera...\n");
                ReadCamera(ml); }
            SkipChunk(obj);
        }
    }
